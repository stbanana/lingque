...
# API_EXTRA_BODY='{"thinking":{"type":"disabled"}}'  # JSON 字符串，原样透传到每次 LLM 调用的 SDK extra_body
...
4. 将 token 写入 `.env`：
   ...
   DISCORD_BOT_TOKEN=xxxxx
   ```
5. （可选，主动私聊必填）拿到你自己的 Discord **User ID**，用于让 bot 主动私聊你（晨报、心跳驱动的主动消息、自主行动）：
   - 在 Discord 打开 **设置 → 高级**，启用 **开发者模式**
   - 右键你自己的头像 → **复制用户 ID**
   - 写入 `~/.lq-{slug}/config.json` 的 `discord.owner_user_id` 字段
   - 确保 bot 与你至少共享一个服务器（Discord 只允许在有共同 guild 的用户之间建立 DM channel）
   - 除非你特意想让 bot 往某个 guild 文字频道发消息，否则保持 `discord.owner_chat_id` 为空
6. 安装可选依赖并启动：
   ```bash
   uv pip install -e '.[discord]'
...
在 Discord 中私聊 bot 或在频道中 @bot 即可对话。自主外发 DM 的实现：adapter 在 connect 时调用 `POST /users/@me/channels` 将 `owner_user_id` 解析成 DM channel 并缓存；入站 DM 也会顺手写入缓存，因此任何私聊过 bot 的用户都可以被 bot 主动 DM 回去。
...
| `api.extra_body` | 供应商特有的请求字段，每次 LLM 调用时原样塞进 SDK 的 `extra_body`。例：`{"thinking": {"type": "disabled"}}`（豆包 / Anthropic 风格关推理）、`{"enable_thinking": false}`（Qwen3 / GLM-Plus）、`{"reasoning_effort": "low"}`（DeepSeek-R1 / o-series）。LingQue 不解读不翻译，写什么后端就收到什么 |
| `feishu.app_id` / `app_secret` | 飞书应用凭证 |
| `discord.bot_token` | Discord bot token（`bot_id` 首次启动时自动填充） |
| `discord.owner_user_id` | 你的 Discord User ID —— adapter 在 connect 时解析成 DM channel，用于自主行动 / 心跳 / 晨报的主动私聊。需要 bot 与你共享至少一个服务器。留空则禁用主动 DM |
| `discord.owner_chat_id` | 可选 guild 文字频道 ID，作为外发目标（替代或附加于 DM）。优先级低于 `owner_user_id` |
